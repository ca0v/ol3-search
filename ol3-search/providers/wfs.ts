...
export class WfsGeocode implements Geocoder<WfsGeocode.WfsRequest, WfsGeocode.WfsResult> {
	...
	private getParameters(options: Request<WfsGeocode.WfsRequest>, map?: ol.Map) {
		...
		if (options.params.query && options.params.searchNames) {
			...
			let filters = options.params.searchNames.map(searchName =>
				olFormatFilter.like(searchName, `*${searchText}*`)
			);
		...
		let getFeatureRequest = <HTMLElement>format.writeGetFeature(<any>{
	...
	private handleResponse(response: WfsGeocode.WfsResponse): Result<WfsGeocode.WfsResult>[] {
		if (!this.options.params.propertyNames) throw "params.propertyNames required";
